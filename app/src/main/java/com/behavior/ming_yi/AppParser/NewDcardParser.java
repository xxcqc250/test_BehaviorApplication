 ...
 */
public class NewDcardParser extends AppTempleteParser {
    private static String TAG="App_Dard";

    public NewDcardParser(Context context, String appname, String event) {
    ...
    public String AppTempleteParser(AccessibilityNodeInfo mAccessibilityNodeInfo) {
        ...
        List<AccessibilityNodeInfo> CacheNodes = mAccessibilityNodeInfo.findAccessibilityNodeInfosByViewId("com.sparkslab.dcard:id/recycler_post");
        List<AccessibilityNodeInfo> CacheMessageNodes = mAccessibilityNodeInfo.findAccessibilityNodeInfosByViewId("com.sparkslab.dcard:id/layout_message");

        if(CacheNodes.size() == 1){
            if (!mAccessibilityNodeInfo.toString().contains("ACTION_SCROLL_BACKWARD")) return null;
            List<AccessibilityNodeInfo> CacheContentNodes = CacheNodes.get(0).findAccessibilityNodeInfosByViewId("com.sparkslab.dcard:id/layout_content");
            if(CacheContentNodes.size() == 0) return null;

            List<AccessibilityNodeInfo> CacheTitleNodes = CacheNodes.get(0).findAccessibilityNodeInfosByViewId("com.sparkslab.dcard:id/text_title");
